...
public class Conta {
    ...
    public void setTipo(int tipo) {
        if (tipo != NORMAL && tipo != ESPECIAL && tipo != INVESTIMENTO) {
    ...
    public void creditarRendimento(double percentualJuros) {
        if (getTipo() == INVESTIMENTO || getTipo() == ESPECIAL) {
            ...
            throw new RuntimeException("Não creditar rendimentos neste tipo de conta");
        }
    }
    ...
    public void depositar(double valorDeposito) {
        if (valorDeposito <= 0) {
            throw new IllegalArgumentException("Valor do depósito dever ser maior que 0");
        }
